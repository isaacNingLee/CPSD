...
def train_cpsd_plus(args, train_data_dir, class_id):
    ...
    denoise_scheduler = DDIMScheduler.from_pretrained(args.pretrained_model_name_or_path, subfolder="scheduler")
    denoise_scheduler.set_timesteps(num_inference_steps=1)
    ...
    latent_trans = torch.nn.Linear(32, 32, bias = True)
    with torch.no_grad():
        latent_trans.weight.copy_(torch.eye(32))  # Set weights to identity matrix
        latent_trans.bias.zero_()  # Set bias to zero


    latent_trans.to(accelerator.device)
    embed_trans.to(accelerator.device)

    # optimize latent_trans and embed_trans
    optimizer = optimizer_cls(
        list(latent_trans.parameters()) + list(embed_trans.parameters()),
        lr=learning_rate,
    ...
    unet, optimizer, train_dataloader, lr_scheduler, embed_trans, latent_trans = accelerator.prepare(
        unet, optimizer, train_dataloader, lr_scheduler, embed_trans, latent_trans
    )
    ...
    denoise_scheduler.alphas_cumprod = denoise_scheduler.alphas_cumprod.to(accelerator.device, dtype=weight_dtype)
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                for i in range(model_pred.shape[0]):
                    denoised_latent = denoise_scheduler.step(model_pred[i], timesteps[i], noisy_latents[i], return_dict=False)[0]
                    denoised_list.append(denoised_latent)

                denoised_latents = torch.stack(denoised_list, dim=0)
                proj_denoised_latents = latent_trans(denoised_latents.reshape(-1, 32)).reshape(denoised_latents.shape)
                proj_denoised_latents = proj_denoised_latents.to(weight_dtype)
                x_hat = vae.decode(proj_denoised_latents / vae.config.scaling_factor).sample
    ...
    if accelerator.is_main_process:
        ...
        }, args.output_dir + f'/cp_embeddings/class_{class_id}.pt')
